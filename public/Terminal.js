export default class Terminal {
  ...
  displayPath() {
    const path = "C:\\Users\\Typing\\Pro\\path";
  ...
  drawSentence() {
    const pathFull = "C:\\Users\\Typing\\Pro\\path:~$";
    ...
    if (this.sentence) {
      ...
      if (
        this.wrongLetter &&
        this.currentIndex + 1 < this.sentence.length - 1
      ) {
        fill(255, 100, 100);
        text(
  ...
  drawCursor() {
    ...
    if (this.sentence) {
      if (Math.floor(frameCount / 60) % 2 === 0) {
        const currentCharWidth = textWidth(
          this.sentence.charAt(this.currentIndex)
        );
